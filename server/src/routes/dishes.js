...
const dishesCollection = getCollectionOperations('dishes')
...
router.get('/', async (req, res) => {
  ...
  const searchableDishes = await dishesCollection.find({})
...
router.put('/:id', async (req, res) => {
  const {
    params: { id },
    body: { _id, ...dishWithoutId },
  } = req

  const response = await dishesCollection.findOneAndReplace(
    { _id: getDBId(id) },
    dishWithoutId
  )
