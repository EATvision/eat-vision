...
setTimeout(() => {
  ...
  // fs.writeFileSync('./server/data/raw/ingredients.json', JSONStream.stringify(ingredients))
}, 10000)
...
setTimeout(() => {
  fs.writeFileSync('./server/data/raw/dishes.json', JSON.stringify(dishes))
}, 10000)
...
base("tbl8Mk37Xtrc6kgyG").select({ view: "Grid view" }).all((_err, records) => {
  const data = records.map((r) => ({
    id: r.getId(),
    ingredient1: r.get("ingredient1_"),
    ...
    dish3_delta_price: r.get("ingredient3"),
...
base("tbl8Mk37Xtrc6kgyG").select({ view: "Grid view" }).all((_err, records) => {
  const data = records.map((r) => ({
    id: r.getId(),
    primer_dishs: r.get("primer_dishs_"),
    Dish1: r.get("Dish1"),
